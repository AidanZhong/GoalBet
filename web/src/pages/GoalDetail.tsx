    ...
    return (
        <div className={"max-w-3xl mx-auto px-4 py-6"}>
            ...
            <div className={"rounded-2xl border border-gray-700 bg-gray-900/70 p-5"}>
                ...
                {/* YouTube declaration */}
                {(goal.youtube_url || isOwner) && (
                    <div className="mt-5">
                        <div className="flex items-center justify-between mb-2">
                            <span className="text-sm text-gray-400">Goal declaration</span>
                            {isOwner && !editingUrl && (
                                <button
                                    onClick={() => { setUrlDraft(goal.youtube_url ?? ""); setEditingUrl(true); }}
                                    className="text-xs text-yellow-400 hover:underline"
                                >
                                    {goal.youtube_url ? "Edit link" : "+ Add link"}
                                </button>
                            )}
                        </div>
                        ...
                        {editingUrl ? (
                            <div className="space-y-2">
                                <input
                                    ...
                                    className="w-full rounded-lg bg-gray-800 border border-gray-700 px-3 py-2 text-white text-sm outline-none focus:border-yellow-400"
                                />
                                {urlDraft.trim() && <YouTubeEmbed url={urlDraft.trim()}/>}
                                <div className="flex gap-2">
                                    <button
                                        ...
                                        onClick={async () => {
                                            setSavingUrl(true);
                                            try {
                                                await goalsService.patchGoal(goal.id, {youtube_url: urlDraft.trim() || null});
                                                await reloadGoal();
                                                setEditingUrl(false);
                                            } catch {
                                        ...
                                        className="rounded-xl bg-yellow-500 hover:bg-yellow-600 text-black font-semibold px-4 py-1.5 text-sm disabled:opacity-60"
                                    >
                                    ...
                                    <button
                                        ...
                                        className="rounded-xl border border-gray-600 px-4 py-1.5 text-sm text-white hover:bg-gray-700 disabled:opacity-60"
                                    >
                                        Cancel
                                    </button>
                                </div>
                            ...
                            goal.youtube_url && <YouTubeEmbed url={goal.youtube_url}/>
                        )}
                    </div>
                )}
